...
import queue_bot.commands.classes.manage_access
import queue_bot.commands.classes.modify_registered
...
class TestRegisteredManager(unittest.TestCase):
    ...
    def test_delete_list(self):
        ...
        tg_select_command(update, queue_bot.commands.classes.modify_registered.ModifyRegistered.RemoveListUsers)
        bot.handle_keyboard_chosen(update, context)

        self.assertIn(students[1], bot.registered_manager.get_users())
        tg_select_command(update, queue_bot.commands.classes.modify_registered.ModifyRegistered.RemoveListUsers, students[1].code_str())
        bot.handle_keyboard_chosen(update, context)
        self.assertNotIn(students[1], bot.registered_manager.get_users())

        self.assertIn(students[2], bot.registered_manager.get_users())
        tg_select_command(update, queue_bot.commands.classes.modify_registered.ModifyRegistered.RemoveListUsers, students[2].code_str())
        bot.handle_keyboard_chosen(update, context)
    ...
    def test_add_remove_admin(self):
        ...
        tg_set_user(update, 1)
        tg_select_command(update, queue_bot.commands.classes.manage_access.ManageAccessRights.AddAdmin)
        bot.handle_keyboard_chosen(update, context)
        ...
        tg_select_command(update, queue_bot.commands.classes.manage_access.ManageAccessRights.RemoveAdmin, test_st)
        bot.handle_keyboard_chosen(update, context)
