...
class ModifyCurrentQueue(CommandGroup):
    ...
    class ShowList(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            ...
            bot.logger.log('showed list from {0} by {1}'.format(str(bot.queues_manager.get_queue()),
    ...
    class ShowQueueForCopy(CommandGroup.Command):
        ...
        def handle(self, update, bot):
            update.effective_chat.send_message(bot.queues_manager.get_queue().get_str_for_copy())
    ...
    class SetStudents(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            bot.logger.log('set students for {0} by {1}'.format(str(bot.queues_manager.get_queue()),
                                                   bot.registered_manager.get_user_by_update(update)))
    ...
    class SetQueuePosition(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            try:
                ...
                bot.request_del()
                bot.logger.log('set queue position from {0} by {1}'.format(str(bot.queues_manager.get_queue()),
                                                   bot.registered_manager.get_user_by_update(update)))
    ...
    class ClearList(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            ...
            bot.logger.log('clear - {0} by {1}'.format(str(bot.queues_manager.get_queue()),
                                                   bot.registered_manager.get_user_by_update(update)))
    ...
    class MoveStudentToEnd(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            move_pos = -1
            try:
                move_pos = int(update.message.text)
                if 0 < move_pos <= len(bot.queues_manager.get_queue()):
                    
                    return

                bot.queues_manager.get_queue().move_to_end(move_pos - 1)
                bot.refresh_last_queue_msg(update)
                ...
                bot.request_del()
                bot.logger.log('moved {2} to end - {0} by {1}'
                               .format(str(bot.queues_manager.get_queue()),
                                       bot.registered_manager.get_user_by_update(update),
                                       move_pos))
    ...
    class RemoveStudentsList(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            positions, errors = parcers.parse_positions_list(update.message.text,
                                                             1, len(bot.queues_manager.get_queue()))
            ...
            bot.request_del()
            bot.logger.log('removed {2} students - {0} by {1}'
                           .format(str(bot.queues_manager.get_queue()),
                                   bot.registered_manager.get_user_by_update(update),
                                   positions))
    ...
    class SetStudentPosition(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            bot.refresh_last_queue_msg(update)
            bot.request_del()
            bot.logger.log('set student position {2} - {0} by {1}'
                           .format(str(bot.queues_manager.get_queue()),
                                   bot.registered_manager.get_user_by_update(update),
                                   positions))
    ...
    class AddMe(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            ...
            bot.queues_manager.get_queue().append_to_queue(student)

            bot.last_queue_message.update_contents(bot.queues_manager.get_queue_str(), update.effective_chat)
            update.message.reply_text(bot.get_language_pack().you_added_to_queue)

            bot.save_queue_to_file()
            bot.logger.log('user added himself - {0} by {1}'
                           .format(str(bot.queues_manager.get_queue()),
                                   bot.registered_manager.get_user_by_update(update)))
    ...
    class RemoveMe(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            student = bot.registered_manager.get_user_by_update(update)
            if student in bot.queues_manager.get_queue():
                bot.queues_manager.get_queue().remove_student(student)

                bot.last_queue_message.update_contents(bot.queues_manager.get_queue_str(), update.effective_chat)
                update.message.reply_text(bot.get_language_pack().you_deleted)

                bot.save_queue_to_file()
                bot.logger.log('removed himself - {0} by {1}'
                               .format(str(bot.queues_manager.get_queue()),
                                       bot.registered_manager.get_user_by_update(update)))
            else:
                update.message.reply_text(bot.get_language_pack().you_not_found)
    ...
    class SwapStudents(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            try:
                ...
                bot.refresh_last_queue_msg(update)
                bot.request_del()
                bot.logger.log('swapped {2} - {0} by {1}'
                               .format(str(bot.queues_manager.get_queue()),
                                       bot.registered_manager.get_user_by_update(update),
                                       positions))
...
class QueuesManage(CommandGroup):
    ...
    class CreateSimple(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            bot.logger.log('set students simple queue - {0} by {1}'
                               .format(str(bot.queues_manager.get_queue()),
                                       bot.registered_manager.get_user_by_update(update)))
    ...
    class CreateRandom(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            QueuesManage.new_queue = queue
            QueuesManage.AddNameToQueue.handle(update, bot)
            bot.logger.log('set students random queue - {0} by {1}'
                           .format(str(bot.queues_manager.get_queue()),
                                   bot.registered_manager.get_user_by_update(update)))
    ...
    class AddNameToQueue(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            bot.logger.log('queue name set {0}'.format(update.message.text))
    ...
    class DefaultQueueName(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            ...
            bot.logger.log('queue set default name')
            QueuesManage.FinishQueueCreation.handle(update, bot)
    ...
    class FinishQueueCreation(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            ...
            bot.save_queue_to_file()
            bot.request_del()
            bot.logger.log('queue set - {0} by {1}'
                           .format(str(bot.queues_manager.get_queue()),
                                   bot.registered_manager.get_user_by_update(update)))
    ...
    class CreateRandomFromRegistered(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            ...
            if not bot.queues_manager.add_queue(queue):
                update.effective_chat.send_message(bot.get_language_pack().queue_limit_reached)
                bot.request_del()
                bot.logger.log('queue limit - {0} by {1}'
                               .format(str(bot.queues_manager.get_queue()),
                                       bot.registered_manager.get_user_by_update(update)))
            else:
                bot.last_queue_message.update_contents(bot.queues_manager.get_queue_str(), update.effective_chat)
                ...
                bot.request_del()
                bot.logger.log('queue added - {0} by {1}'
                               .format(str(bot.queues_manager.get_queue()),
                                       bot.registered_manager.get_user_by_update(update)))
        ...
        def handle(cls, update, bot):
            ...
            if queue_name is not None:
                if bot.queues_manager.remove_queue(queue_name):
                    ...
                    bot.logger.log('queue not found, query: {2} - {0} by {1}'
                                   .format(str(bot.queues_manager.get_queue()),
                                           bot.registered_manager.get_user_by_update(update),
                                           update.callback_query.data))
            else:
                bot.logger.log('request {0} in {1} has no arguments'
                               .format(update.callback_query.data, cls.__qualname__))
    ...
    class ChooseOtherQueue(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            ...
            if queue_name is not None:
                ...
                bot.logger.log('request {0} in {1} has no arguments'.format(update.callback_query.data, cls.__qualname__))
...
class ModifyRegistered(CommandGroup):
    ...
    class AddListUsers(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            bot.request_del()
            bot.logger.log('added users: {1} - {1}'
    ...
    class AddUser(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            if update.message.forward_from is not None:
                ...
                bot.save_registered_to_file()
                bot.logger.log('added one user: {2} - {0} by {1}'
                               .format(str(bot.queues_manager.get_queue()),
                                       bot.registered_manager.get_user_by_update(update),
    ...
    class RemoveListUsers(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            delete_indexes, errors = parcers.parse_positions_list(update.message.text,
            ...
            if len(delete_indexes) > 0:
                update.effective_chat.send_message(bot.get_language_pack().users_deleted)
            bot.request_del()
            bot.logger.log('removed users {1} - {0}'
                           .format(bot.registered_manager.get_user_by_update(update),
                                   delete_indexes))
    ...
    class MovePrevious(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            if bot.queues_manager.get_queue().move_prev():
                ...
                bot.last_queue_message.update_contents(bot.queues_manager.get_queue_str(), update.effective_chat)
                bot.logger.log('moved previous - {0} : {1}'.format(update.effective_user.full_name,
    ...
    class MoveNext(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            if bot.queues_manager.get_queue().move_next():
                bot.send_cur_and_next(update)
                bot.last_queue_message.update_contents(bot.queues_manager.get_queue_str(), update.effective_chat)
                bot.logger.log('moved queue - {0} : {1}'.format(update.effective_user.full_name,
                                                          update.effective_user.id))
    ...
    class Refresh(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            if not bot.last_queue_message.message_exists(update.effective_chat):
                update.effective_message.delete()
            bot.last_queue_message.update_contents(bot.queues_manager.get_queue_str(), update.effective_chat)
            bot.logger.log('refreshed queue - {0} : {1}'.format(update.effective_user.full_name,
                                                          update.effective_user.id))
...
class ManageUsers(CommandGroup):
        ...
        def handle_request(cls, update, bot):
            if update.message.forward_from is not None:
                if bot.registered_manager.set_admin(update.effective_user.id):
                    ...
                    bot.save_registered_to_file()
                    bot.logger.log('added admin {1} - {0}'
                                   .format(bot.registered_manager.get_user_by_update(update),
                                           update.message.forward_from.full_name))
            else:
                update.message.reply_text(bot.get_language_pack().was_not_forwarded)
                bot.logger.log('admin message not forwarded - {0}'
                               .format(bot.registered_manager.get_user_by_update(update)))
            bot.request_del()
        ...
        def handle_request(cls, update, bot):
            if update.message.forward_from is not None:
                if bot.registered_manager.set_user(update.message.forward_from.id):
                    ...
                    bot.logger.log('deleted admin {1} - {0}'
                                   .format(bot.registered_manager.get_user_by_update(update),
                                           update.message.forward_from.full_name))
            else:
                update.message.reply_text(bot.get_language_pack().was_not_forwarded)
    ...
    class AddUsersList(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            bot.save_registered_to_file()
            bot.request_del()
            bot.logger.log('users list added - {0}'
                           .format(bot.registered_manager.get_user_by_update(update)))
...
class CollectSubjectChoices(CommandGroup):
    ...
    class CreateNewCollectFile(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            if ' ' in subject_name:
                ...
                update.effective_chat.send_message(bot.get_language_pack().value_set)
                bot.logger.log('start setting new subject - {0}'
                               .format(bot.registered_manager.get_user_by_update(update)))
    ...
    class SetSubjectsRange(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            if min_range is None:
                ...
                bot.logger.log('set number interval')
    ...
    class SetRepeatLimit(CommandGroup.Command):
        ...
        def handle_request(cls, update, bot):
            ...
            if limit is None:
                ...
                update.effective_chat.send_message(bot.get_language_pack().value_set)
                bot.logger.log('set repeat limit')
    ...
    class FinishSubjectChoiceCreation(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            ...
            bot.request_del()
            bot.logger.log('finished new subject creation')
    ...
    class RemoveChoice(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            if not bot.choice_manager.can_choose:
                update.effective_chat.send_message(bot.get_language_pack().choices_collection_not_started)
                bot.logger.log('requested remove while choice not started - {0}'
                               .format(bot.registered_manager.get_user_by_update(update)))
                return
            ...
            bot.logger.log('choice removed - {0}'
                           .format(bot.registered_manager.get_user_by_update(update)))
    ...
    class StartChoose(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            ...
            if not bot.choice_manager.start_choosing():
                ...
                bot.logger.log('cannot start choice, must perform setup - {0}'
                               .format(bot.registered_manager.get_user_by_update(update)))
            else:
                bot.logger.log('subject choice started - {0}'
                               .format(bot.registered_manager.get_user_by_update(update)))
    ...
    class StopChoose(CommandGroup.Command):
        ...
        def handle(self, update, bot):
            if not bot.choice_manager.can_choose:
                update.effective_chat.send_message(bot.get_language_pack().choices_collection_not_started)
                bot.logger.log('requested stop while choice not started - {0}'
                               .format(bot.registered_manager.get_user_by_update(update)))
                return
            ...
            bot.logger.log('subject choice stopped - {0}'
                           .format(bot.registered_manager.get_user_by_update(update)))
    ...
    class ShowCurrentChoices(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            if bot.choice_manager.current_subjects is not None:
                ...
                bot.logger.log('requested show choices - {0}'
                               .format(bot.registered_manager.get_user_by_update(update)))
            else:
                update.effective_chat.send_message(bot.get_language_pack().choices_collection_not_started)
    ...
    class GetExcelFile(CommandGroup.Command):
        ...
        def handle(cls, update, bot):
            ...
            try:
                ...
                bot.logger.log('cannot send choice document to {0}'
                               .format(bot.registered_manager.get_user_by_update(update)))
