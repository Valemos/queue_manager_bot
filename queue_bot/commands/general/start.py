...
class Start(Command):
    ...
    def handle_request(cls, update, bot):
        ...
        if not bot.registered_manager.exists_user_access(AccessLevel.GOD):
            bot.registered_manager.add_new_user(update.message.from_user.username, update.message.from_user.id)
            bot.registered_manager.set_god(update.message.from_user.id)
            update.message.reply_text(language_pack.first_user_added.format(update.message.from_user.username))

            for admin in update.effective_chat.get_administrators():
                bot.registered_manager.add_new_user(admin.user.username, admin.user.id)
                bot.registered_manager.set_admin(admin.user.id)

            bot.save_registered_to_file()
            update.effective_chat.send_message(language_pack.admins_added)
        else:
            update.message.reply_text(language_pack.bot_already_running)